/* =========================================================================
   Ludora — profile.js
   Player progression engine: XP/level curves, gameplay statistics,
   achievement unlocks, cosmetic customization, and deterministic daily
   challenges. All state persists offline via store.js.
   ========================================================================= */
(function (global) {
  'use strict';
  ...
  function levelFromXp(xp) {
    var lvl = 1, rem = Math.max(0, xp || 0);
    while (rem >= xpForNext(lvl)) {
      rem -= xpForNext(lvl);
      lvl++;
      if (lvl > 999) break;
    }
  ...
  /* ---------- Avatars ---------- */
  var AVATARS = [
    { id: 0, c1: '#FF6B6B', c2: '#C64444' },
    { id: 1, c1: '#4ECDC4', c2: '#2B9D8F' },
    { id: 2, c1: '#FFD93D', c2: '#D9A514' },
    { id: 3, c1: '#6A8DFF', c2: '#3D5CD6' },
    { id: 4, c1: '#B28DFF', c2: '#7A5BC7' },
    { id: 5, c1: '#FF9F68', c2: '#D6552B' },
    { id: 6, c1: '#7ED957', c2: '#3F9A2E' },
    { id: 7, c1: '#F472B6', c2: '#C2417E' }
  ];
  ...
  var COSMETICS = {
    boards: [
      ...
      { id: 'walnut',   name: 'Walnut Wood',    unlock: { level: 4 } },
      { id: 'midnight', name: 'Midnight Glass', unlock: { level: 7 } },
      { id: 'sakura',   name: 'Sakura Blossom', unlock: { level: 10 } },
      { id: 'cyber',    name: 'Cyber Neon',     unlock: { level: 12 } },
      { id: 'arctic',   name: 'Arctic Frost',   unlock: { level: 14 } },
      { id: 'canyon',   name: 'Canyon Sunset',  unlock: { level: 16 } },
      { id: 'emerald',  name: 'Emerald Forest', unlock: { level: 19 } },
      { id: 'aurora',   name: 'Aurora Borealis',unlock: { level: 22 } },
      { id: 'royal',    name: 'Royal Velvet',   unlock: { ach: 'wins-50' } },
      { id: 'obsidian', name: 'Obsidian Chrome',unlock: { ach: 'mastermind' } }
    ],
    dice: [
      { id: 'ivory',    name: 'Classic Ivory', unlock: null },
      { id: 'obsidian', name: 'Obsidian Onyx', unlock: { level: 5 } },
      { id: 'crimson',  name: 'Crimson Ruby',  unlock: { level: 8 } },
      { id: 'jade',     name: 'Jade Emerald',  unlock: { level: 11 } },
      { id: 'gold',     name: 'Golden Sun',    unlock: { level: 15 } },
      { id: 'galaxy',   name: 'Galaxy Nebula', unlock: { ach: 'sixes-100' } },
      { id: 'amethyst', name: 'Royal Amethyst',unlock: { ach: 'daily-7' } }
    ],
    tokens: [
      { id: 'classic', name: 'Classic Pawn',   unlock: null },
      { id: 'orb',     name: 'Crystal Orb',    unlock: { level: 3 } },
      { id: 'gem',     name: 'Faceted Gem',    unlock: { level: 6 } },
      { id: 'cyber',   name: 'Cyber Mech',     unlock: { level: 13 } },
      { id: 'regal',   name: 'Regal Crown',    unlock: { ach: 'mastermind' } },
      { id: 'star',    name: 'Star Crest',     unlock: { ach: 'captures-250' } }
    ]
  };
  ...
  function isUnlocked(item, profile) {
    ...
    if (item.unlock.ach) return !!(profile.achievements && profile.achievements[item.unlock.ach]);
    return false;
  }
  ...
  var ACHIEVEMENTS = [
    ...
    { id: 'collector',   name: 'Collector',      desc: 'Own 8 cosmetics',               cond: function (c) { return c.profile.cosmetics && c.profile.cosmetics.owned && c.profile.cosmetics.owned.length >= 8; } },
    { id: 'tutorial',    name: 'First Steps',    desc: 'Complete the interactive tutorial', cond: function (c) { return !!c.profile.tutorialCompleted; } },
    { id: 'grandmaster', name: 'Grandmaster',    desc: 'Reach Player Level 20',         cond: function (c) { return levelFromXp(c.profile.xp).level >= 20; } }
  ];
  ...
  var DAILY_TYPES = [
    ...
    },
    {
      ...
      build: function (rng, names) {
        return {
          rules: { headStart: { 1: [12, 8, 5, 0], 3: [12, 8, 5, 0] } },
          seats: [
            { color: 0, kind: 'human', name: 'You' },
  ...
  function dailyFor(dateStr) {
    ...
    var names = (global.LudoraAI && global.LudoraAI.names ? global.LudoraAI.names.slice() : ['Aria', 'Rohan', 'Mila', 'Kabir', 'Zara', 'Dev', 'Nina', 'Arjun', 'Tara', 'Vikram']);
    ...
    return {
      ...
      type: type.id,
      name: type.name,
      desc: type.desc({ aiName: cfg.seats[1].name }),
      seats: cfg.seats,
      rules: cfg.rules
    };
  }
  ...
  function dailyReward(streak) {
    return 150 + 20 * Math.min(streak || 0, 7);
  }
  ...
  function defaultProfile() {
    return {
      ...
      achievements: {},
      cosmetics: {
        owned: ['ivory-b', 'ivory-d', 'classic-t'],
        board: 'ivory',
        dice: 'ivory',
        token: 'classic'
      },
  ...
  function validateProfile(p) {
    ...
    if (p.settings.theme != null && ['auto', 'light', 'dark'].indexOf(p.settings.theme) < 0) {
  ...
  function loadProfile() {
    ...
    if (!p) {
      p = defaultProfile();
      Store.save(Store.keys.profile, p);
    }
    return p;
  }
  ...
  function saveProfile(p) {
    p.v = 2;
    return Store.save(Store.keys.profile, p);
  }

  /* ---------- Match Progression Application ---------- */
  function applyMatchResult(profile, match) {
    ...
    profile.settings = Object.assign({}, d.settings, profile.settings);
    ...
    if (countsForYou) {
      if (won) {
        ...
        if (match.you) {
          profile.stats.captures += match.you.captures || 0;
          profile.stats.timesCaptured += match.you.timesCaptured || 0;
          profile.stats.sixes += match.you.sixes || 0;
          profile.stats.homes += match.you.homes || 0;
        }
      ...
      if (match.you) {
        var capXp = Math.min(24, (match.you.captures || 0) * 4);
        if (capXp) addXp(out, capXp, match.you.captures + ' captures');
        var sixXp = Math.min(12, match.you.sixes || 0);
    ...
    if (match.tutorial) {
      ...
      addXp(out, 100, 'Tutorial Completed');
    }
    ...
    if (match.winnerName) {
      profile.history.unshift({
        ...
        seats: match.seatNames || [],
        seatCount: match.seatCount || 2,
        captures: match.you ? match.you.captures : null,
        durationS: match.durationS || 0
      });
