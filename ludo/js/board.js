...
(function (global) {
  ...
  var PLAYERS = [
    { name: 'Red',    base: '#D6453F', light: '#F5897F', lighter: '#FFD3CC', dark: '#9C241F', deep: '#6E1210' },
    { name: 'Green',  base: '#2FA35F', light: '#77CD9C', lighter: '#D2F5DF', dark: '#1C6E3E', deep: '#0E4A28' },
    { name: 'Yellow', base: '#EDA62F', light: '#F9CB7B', lighter: '#FDEBC8', dark: '#B47212', deep: '#7C4A08' },
    { name: 'Blue',   base: '#3F7BE0', light: '#86AEF2', lighter: '#D2E2FC', dark: '#2854A8', deep: '#163572' }
  ];

  var THEMES = {
    ivory:    { name: 'Classic Ivory', frameA: '#6E4A2F', frameB: '#3B2817', field: '#EFE5CC', cell: '#F9F3E1', line: 'rgba(96,74,42,.16)', yardLine: 'rgba(0,0,0,.20)' },
  ...
  function drawStatic(ctx, m, themeId) {
    ...
    /* safe stars: stars on the 4 neutral safe cells, white stars on starts */
    [8, 21, 34, 47].forEach(function (idx) {
      var cc = E.RING[idx];
      star4(ctx, cx(m, cc[0]), cy(m, cc[1]), cell * 0.30, cell * 0.105);
      ctx.lineWidth = Math.max(1.5, cell * 0.055); ctx.lineJoin = 'round';
      ctx.strokeStyle = 'rgba(70,54,30,.42)'; ctx.stroke();
    });
    ...
    tris.forEach(function (t) {
      ...
      ctx.fillStyle = lg(ctx, t[0], t[1], mxv, myv, [[0, t[4].base], [1, t[4].dark]]);
      ctx.fill();
    ...
    ctx.fillStyle = rg(ctx, mxv, myv, S * 0.2, mxv, myv, S * 0.78,
      [[0, 'rgba(0,0,0,0)'], [0.75, 'rgba(0,0,0,0)'], [1, 'rgba(20,12,4,.10)']]);
    ctx.fill();
  }
  ...
  function drawToken(ctx, x, y, r, colorIdx, shape, o) {
    ...
    ctx.fillStyle = 'rgba(24,15,6,' + (0.30 * (1 - lift * 0.45)).toFixed(3) + ')';
    ctx.fill();
